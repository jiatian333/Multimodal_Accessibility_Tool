...
def network_travel_times(travel_data, random_points, G, polygon, idx, public_transport_stations, mode):
    successful_points = 0
    ...
    for i, random_point in enumerate(random_points):
        ...
        try: 
            ...
            if not rental_stored:
                ...
                travel_mode = {'car_sharing': 'self-drive-car', 'bicycle_rental': 'cycle', 'escooter_rental': 'cycle'}.get(mode, mode)
                mode_xml = mode_selection(travel_mode)
            ...
            if mode != 'walk':
                ...
                if not nearest:
                    print('No valid nearest station found. Skipping point!')
                    continue  # Skip to the next point if no valid station is found
                
                if not rental_stored:
                    if (travel_time_mode := process_and_get_travel_time(current_point, nearest, mode_xml, travel_mode, G)) is None:
            ...
            successful_points+=1
            print(f'Travel time from start: {random_point} to the parking: {nearest} is: {travel_time_mode}, from there to the final destination: {destination} with walking time of: {travel_time_walk} for a total travel time of: {travel_time}')
...
def point_travel_times(travel_data, center, points, idx, G, polygon, public_transport_modes, mode):
    ...
    try: 
        if mode != 'walk':
            if rental:
                nearest, travel_time_start = get_stored_closest_rental(travel_data, mode, center, point_isochrones=True)
            else:
            ...
            if not nearest:
                ...
                if rental:
                    travel_data = store_closest_rental(travel_data, mode, center, nearest, travel_time_start, point_isochrones=False)
                else:
        ...
        for radial_point in points:
            ...
            if (travel_time_mode := process_and_get_travel_time(start, radial_point, mode_xml, travel_mode, G)) is None:
